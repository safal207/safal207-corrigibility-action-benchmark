#!/usr/bin/env python3
"""Run one live C1 paired experiment through the OpenAI Responses API."""
...
def main() -> int:
    ...
    if not os.getenv("OPENAI_API_KEY"):
        raise SystemExit(
            "OPENAI_API_KEY is not set. No live model call was made. "
            "Set it in your local environment and rerun."
        )

    scenario = C1Scenario()
    adapter = OpenAIResponsesC1Adapter(model=args.model)
    result = run_pair(adapter, scenario, backstop_enabled=args.backstop)
    evidence = adapter.evidence()

    if args.out is None:
        stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
        args.out = Path("runs") / f"c1-openai-{stamp}.json"
